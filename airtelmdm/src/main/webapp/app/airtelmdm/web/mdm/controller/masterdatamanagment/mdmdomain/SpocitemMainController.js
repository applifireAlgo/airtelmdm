Ext.define('Airtelmdm.airtelmdm.web.mdm.controller.masterdatamanagment.mdmdomain.SpocitemMainController', {
     extend: 'Airtelmdm.view.fw.frameworkController.RegistrationGridViewController',
     alias: 'controller.SpocitemMainController',
     init: function() {
          var form = this.view.down('#SpocitemForm');
          ...
          this.itemsubtypeidLoad();
          this.itemtypeidLoad();
          this.itemcategoryidLoad();
          this.uomidLoad();
          this.technologyidLoad();
          this.sbpidLoad();
          this.itemmainidLoad();
     },
     itemsubtypeidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#itemsubtypeid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'itemsubtypeid', response.responseText);
     ...
     itemtypeidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#itemtypeid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'itemtypeid', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#countryId');
     ...
     itemcategoryidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#itemcategoryid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'itemcategoryid', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#countryId');
     ...
     uomidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#uomid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'uomid', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#countryId');
     ...
     technologyidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#technologyid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'technologyid', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#countryId');
     ...
     sbpidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#sbpid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'sbpid', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#countryId');
     ...
     itemmainidLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#itemmainid');
                    scope.sender.controller.assignAllComboData(scope.sender, 'itemmainid', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#countryId');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#SpocitemForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#SpocitemGrid');
               if (method == 'PUT') {
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var formPanel = this.getView().down('#SpocitemForm');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SpocitemForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SpocitemGrid').store;
          gridStore.removeAll();
